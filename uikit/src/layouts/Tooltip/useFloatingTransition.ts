...
type CSSStylesProperty = React.CSSProperties | ((side: Side) => React.CSSProperties);
...
export function useFloatingTransition<RT extends ReferenceType = ReferenceType>(
  ...
  useLayoutEffect(() => {
    ...
    if (open) {
      const frame = requestAnimationFrame(() => {
        const toValue =
          typeof toWithFallback === 'function' ? toWithFallback(side) : toWithFallback;
        setStyles({
  ...
  }, [open, refs, placement, side, openDuration, closeDuration, fromRef, exitRef, toRef]);
