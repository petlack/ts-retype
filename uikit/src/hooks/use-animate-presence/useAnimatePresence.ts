...
import { createSpringAnimation, SpringParameters } from './spring';
...
export type SpringOptions = Pick<SpringParameters, 'damping' | 'stiffness' | 'mass'>;
...
const isAnyDefined = (...args: unknown[]) => args.some((a) => !isUndefined(a));
const isRunning = (anim?: Animation) => anim && anim.playState === 'running';

const startAnimation = (el: Element, keyframes: Keyframe[], duration: number) => {
...
export const useAnimatePresence = ({
  ...
  const animateSpring = ({ el, visible, duration = 1000 }: AnimateSpringArgs) => {
    ...
    keyframes[keyframes.length - 1].opacity = visible ? opacity.to : opacity.from;

    return startAnimation(el, keyframes, noMove ? duration : (frames / 60) * 1000);
  };

  const animateVisible = (el: Element) =>
    animateSpring({
      el,
      duration,
      visible: true,
  ...
  const handleExitAnimationEnd = (exitCb: () => void) => {
