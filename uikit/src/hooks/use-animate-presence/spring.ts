...
export function createSpringAnimation({
  ...
  for (let step = 0; step <= 1000; step += 1) {
    const Fspring_x = k * (x - spring_length);
    const Fspring_y = k * (y - spring_length);
    const Fspring_r = k * (r - spring_length);
    const Fspring_s = k * (s - spring_length);
    const Fdamping_x = d * velocity_x;
    const Fdamping_y = d * velocity_y;
    const Fdamping_r = d * velocity_r;
    const Fdamping_s = d * velocity_s;

    const accel_x = (Fspring_x + Fdamping_x) / mass;
    const accel_y = (Fspring_y + Fdamping_y) / mass;
    const accel_r = (Fspring_r + Fdamping_r) / mass;
    const accel_s = (Fspring_s + Fdamping_s) / mass;
    ...
    keyframes.push({
      transform: `translate(${x}px, ${y}px) rotate(${r}deg) scale(${s / scale + 1})`,
    });
    ...
    if (frames_below_threshold >= 60) {
      console.debug('Largest displacement over last 60 frames', `${Math.abs(largest_displ)}px`);
      frames = step;
      break;
    }
  }
