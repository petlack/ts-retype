...
export function getCodeSnippet(
    ...
    let leftOffset = 0;
    while (
        startAt + leftOffset >= 0 &&
        fulltext.at(startAt + leftOffset) !== '\n'
    ) {
        leftOffset--;
    }
    ...
    leftOffset = leftOffset + startAt < 0 ?
        -1 * startAt :
        leftOffset;
    rightOffset = endBefore + rightOffset > fulltext.length ?
        fulltext.length - endBefore :
        rightOffset;

    const nonEmptyLines = slice(
        startAt + leftOffset,
        endBefore + rightOffset,
        fulltext,
    );
