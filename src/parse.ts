...
export function getAllLiteralTypes(sourceFile: ts.SourceFile) {
  ...
  visitTopLevelDeclarations(sourceFile, function (node: any) {
    ...
    case ts.SyntaxKind.InterfaceDeclaration: {
      ...
      all.push({ name, type: 'interface',  pos: [node.pos, node.end], properties: interfaceProperties });
      return true;
    }
    case ts.SyntaxKind.TypeAliasDeclaration: {
      if (node.type && node.type.members) {
        ...
        all.push({ name, type: 'alias', pos: [node.pos, node.end], properties: typeProperties });
        return true;
      } else {
        return false;
      }
    ...
    case ts.SyntaxKind.TypeLiteral: {
      ...
      all.push({ name: 'anonymous', type: 'literal', pos: [node.pos, node.end], properties: children });
      return true;
    }
    default:
      return false;
