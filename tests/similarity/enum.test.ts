...
describe('enum type', () => {
  test('same names, no members', () => {
    const given = similarity(
      { name: 'foo', type: 'enum', members: [], pos: [0, 0] } as EnumCandidateType,
      { name: 'foo', type: 'enum', members: [], pos: [0, 0] } as EnumCandidateType,
    );
  ...
  test('identical types with no properties and different names', () => {
    const given = similarity(
      { name: 'foo', type: 'enum', pos: [0, 0], members: [] } as EnumCandidateType,
      { name: 'bar', type: 'enum', pos: [0, 0], members: [] } as EnumCandidateType,
    );
    expectSimilarity(given, Similarity.HasIdenticalProperties);
  });
});
